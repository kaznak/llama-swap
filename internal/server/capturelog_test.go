...
// (captureBuffer 0) and whose JSONL sink writes to a temp file, proving the
// sink does not depend on the ring. Returns the monitor and the sink path.
...
func readCaptureLog(t *testing.T, mm *metricsMonitor, path string) []captureLogRecord {
	t.Helper()
	if err := mm.Close(); err != nil {
...
func TestCaptureLog_SuccessIsByteExact(t *testing.T) {
	mm, path := captureLogSink(t, config.CaptureLogConfig{Enabled: true})
...
// TestCaptureLog_DisabledWritesNothing checks the default: no file, no
...
func TestCaptureLog_DisabledWritesNothing(t *testing.T) {
	...
	mm.attachCaptureLog(config.CaptureLogConfig{Enabled: false, Path: path})
	...
	if _, err := os.Stat(path); !os.IsNotExist(err) {
		t.Fatalf("capture log %s exists (stat err = %v); a disabled sink must not create it", path, err)
	}
...
func TestCaptureLog_AbortedRequest(t *testing.T) {
	...
	t.Run("excluded by default", func(t *testing.T) {
		...
		if len(data) != 0 {
			t.Fatalf("want an empty capture log, got %q", data)
		}
	})

	t.Run("included when asked", func(t *testing.T) {
		mm, path := captureLogSink(t, config.CaptureLogConfig{Enabled: true, IncludeAborted: true})
		abort(t, mm)
		recs := readCaptureLog(t, mm, path)
		if len(recs) != 1 {
			t.Fatalf("want 1 line, got %d", len(recs))
